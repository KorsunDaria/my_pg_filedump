...
/* добавить что корректно не валидно в вывод количество страницы по категориям от 0 до 255 */
/* pg indent */
...
static FsmPageInfo * load_fsm(const char *path,
							  long *out_total_pages)
{
...
static void
long_vec_push(LongVec * v, long x)
{
	if (v->count == v->cap)
	{
...
static void
byte_compressed_range_push(ByteCompressedRangeVec * v, long start,
						   long end, uint8 value)
{
	...
	v->items[v->count++] = (ByteCompressedRange)
	{
		start, end, value
	};
}
...
static void
byte_diff_compressed_range_push(ByteDiffCompressedRangeVec * v,
								...
								uint8 nv)
{
	...
	v->items[v->count++] = (ByteDiffCompressedRange)
	{
		start, end, ov, nv
	};
}
...
/* !good look */
static void
print_leaf_compressed(FILE *out,
					  const char *indent,
					  const FsmPageInfo * page_info,
...
static int
do_fsm_dump(const char *in_path, const char *out_path,
			const FsmOptions * options)
{
	long		total_pages;
...
static void
leaf_delta_push(LeafDeltaVec * v, long start, long end, long delta)
{
	if (v->count == v->cap)
	{
		v->cap = v->cap ? v->cap * 2 : 16;
				...
				LeafDeltaVec * deltas)
{
	...
	for (long i = 0; i < compressed.count; i++)
	{
		...
		if (r->old_value == r->new_value)
			continue;
		unsigned	ob = cat_to_bytes(r->old_value),
...
static void
diff_node(FILE *out, FsmPageInfo * A, long totalA, FsmPageInfo * B,
		  ...
		  LeafDeltaVec * deltas)
{
	...
	{
		stats->pages_changed++;
		/* !old_page->header  new_page->header */
...
static int
do_fsm_diff(const char *old_path, const char *new_path,
